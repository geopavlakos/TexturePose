...
def flip_img(img):
    # flips rgb images or masks
    # channels come last, e.g. (256,256,3)
    img = np.fliplr(img)
    return img

def flip_bbox(bbox):
    # flips bbox (in normalized coordinates)
    bbox[[0,2]] = -bbox[[2,0]]
    return bbox

def flip_corners(corners):
    # flips corners (in normalized coordinates)
    corners[:,0] = -corners[[1,0,3,2],0]
    return corners

def flip_kp(kp):
    # flips keypoints in normalized coordinates
    flippedParts = [5, 4, 3, 2, 1, 0, 11, 10, 9, 8, 7, 6, 12, 13]
    kp = kp[flippedParts]
    kp[:,0] = -kp[:,0]
    return kp
...
def flip_pose(pose):
    # flips pose
    # the flipping is purely based on SMPL
...
def uncrop(img, center, scale, res, orig_shape, rot=0, is_rgb=True):
    # Upper left point
    ul = np.array(transform([1, 1], center, scale, res, invert=1))-1
    # Bottom right point
